        ...
        B: Into<Body> + Clone,
    {
        ...
        let mut video = match self.line {
            Bucket::Cos(bucket, enable_internal) => {
                ...
                let cos_client = Cos::form_post(client, bucket, counter, on_progress).await?;
            ...
            Bucket::Kodo(bucket) => {
                ...
                Kodo::from(client, bucket, counter, on_progress)
                    .await?
                    .upload_stream(
                        progress(self.video_file.get_stream(chunk_size)?),
                        self.video_file.total_size,
            ...
            Bucket::Upos(bucket) => {
                ...
                let upos = Upos::from(client, bucket, counter, on_progress).await?;
