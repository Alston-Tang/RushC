...
impl Cos {
    pub async fn form_post(client: StatelessClient, bucket: Bucket, counter: Arc<AtomicUsize>, on_progress: fn(Arc<AtomicUsize>)->()) -> Result<Cos> {
        ...
        let stream = stream
            ...
            .map(move |(i, chunk)| async move {
                ...
                let response = retry(|| async {
                    ...
                    counter.fetch_add(len, Relaxed);
                    on_progress(counter.clone());
