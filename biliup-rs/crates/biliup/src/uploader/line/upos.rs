...
pub struct Upos {
    ...
    on_progress: fn(Arc<AtomicUsize>)->()
}
...
impl Upos
{
    pub async fn from(client: StatelessClient, bucket: Bucket, counter: Arc<AtomicUsize>, on_progress: fn(Arc<AtomicUsize>)->()) -> Result<Self> {
        let url = format!(
            "https:{}/{}",
            bucket.endpoint,
        ...
        let stream = stream
            ...
            .map(move |(i, chunk)| async move {
                ...
                retry(|| async {
                    ...
                    self.counter.fetch_add(len, Relaxed);
                    on_progress(self.counter.clone());
                    Ok::<_, Kind>(())
                })
                .await?;
